...
/// _consume_one_job topology, and _poll_warm_queue_depths poller).</summary>
    ...
    JobsMetrics metrics, ILogger<WarmConsumer> logger)
{
    ...
    {
        // Concurrency comes from RabbitMQ's consumer dispatcher (default 1 = serial). Compute the max
        // prefetch across jobs up front so the factory + QoS are sized before any consumer starts.
        var maxPrefetch = config.PrefetchDefault;
        foreach (var job in jobs)
        {
            var pol = job.TryGetProperty("policy", out var pp) ? pp : default;
            maxPrefetch = Math.Max(maxPrefetch, PolicyInt(pol, "concurrency", config.PrefetchDefault));
        }

        var factory = new ConnectionFactory
        {
            Uri = new Uri(config.AmqpUrl),
            // ConsumerDispatchConcurrency on the factory sets a default for all channels
            // created from the connection. We also set it per-channel via CreateChannelOptions
            // to be explicit and compatible with RabbitMQ.Client 7.x.
            ConsumerDispatchConcurrency = (ushort)maxPrefetch,
        };
        await using var connection = await factory.CreateConnectionAsync(ct);

        // RabbitMQ.Client 7.x: CreateChannelAsync takes CreateChannelOptions — no bare CancellationToken overload.
        // CreateChannelOptions is a readonly struct; ConsumerDispatchConcurrency is set via the 4-arg constructor
        // (publisherConfirmationsEnabled, publisherConfirmationTrackingEnabled, rateLimiter, consumerDispatchConcurrency).
        ...
        await channel.ExchangeDeclareAsync(QueueNames.ExchangeName, ExchangeType.Topic, durable: true,
            autoDelete: false, cancellationToken: ct);
        // QoS must be applied BEFORE consumers start, or the prefetch limit has no effect.
        await channel.BasicQosAsync(0, (ushort)maxPrefetch, global: false, ct);

        var publisher = new RabbitRetryPublisher(channel);
        ...
        foreach (var job in jobs)
        {
            ...
            await channel.QueueDeclareAsync(work, durable: true, exclusive: false, autoDelete: false, cancellationToken: ct);
            ...
            await channel.QueueDeclareAsync(dead, durable: true, exclusive: false, autoDelete: false, cancellationToken: ct);
            await channel.QueueBindAsync(work, QueueNames.ExchangeName, work, cancellationToken: ct);
            await channel.QueueBindAsync(retry, QueueNames.ExchangeName, retry, cancellationToken: ct);
            await channel.QueueBindAsync(dead, QueueNames.ExchangeName, dead, cancellationToken: ct);
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                ...
                try
                {
                    var delivery = new RabbitDelivery(channel, ea);
            ...
            await channel.BasicConsumeAsync(work, autoAck: false, consumer: consumer, cancellationToken: ct);
        }
        ...
        Heartbeat heartbeat, CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            try
            {
                // Polling channel: no publisher confirmations or concurrent dispatch needed.
...
/// <summary>Publishes envelopes to the retry / dead queues. Port of _publish_to_retry/_publish_to_dlq.</summary>
